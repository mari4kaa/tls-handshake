...
export class HandshakeController {
  ...
  @Post('client-hello')
  async handleClientHello(@Body() body: { nodeId: string; clientRandom: string }) {
    return await this.handshakeService.handleClientHello(body.nodeId, body.clientRandom);
  }
  ...
  async initiateHandshake(
    ...
    this.logger.log(`Received handshake initiation request for ${body.targetNodeId}`);
    ...
    try {
      ...
      this.logger.log(`✓ Handshake completed successfully with ${body.targetNodeId}`);
      ...
      this.logger.error(`✗ Handshake failed with ${body.targetNodeId}:  ${error.message}`);
