...
export class CaController {
  ...
  async verifyCertificate(@Body() certificate: Certificate) {
    this.logger.log(
      `→ Certificate verification request:  ${certificate.serialNumber}`
    );
  ...
  getIssuedCertificates() {
    ...
    this.logger.log(
      `→ Certificate registry requested (${certificates.length} certificates)`
    );
  ...
  @HttpCode(200)
  revokeCertificate(@Body() body: { serialNumber: string; reason: string }) {
