...
export class CaAuthorityService {
  ...
  async signCertificate(csr: CertificateSigningRequest): Promise<Certificate> {
    ...
    this.logger.log(
      `  Signature Length: ${certificate.signature.length} chars`
    );
    this.logger.log(
      `  Total Certificates Issued: ${this.issuedCertificates.size}`
    );
    this.logger.log(
      "─────────────────────────────────────────────────────────"
    );
  ...
  private validateCSR(csr: CertificateSigningRequest): void {
    ...
    const existingCert = Array.from(this.issuedCertificates.values()).find(
      (cert) => cert.issuedTo === csr.nodeId
    );

    if (existingCert && !this.isCertificateExpired(existingCert)) {
      this.logger.warn(
        `⚠ Node ${csr.nodeId} already has valid certificate (serial: ${existingCert.serialNumber})`
      );
  ...
  revokeCertificate(serialNumber: string, reason: string): void {
    ...
    this.logger.log(
      `  Total revoked certificates: ${this.revokedSerialNumbers.size}`
    );
    this.logger.log("");
  }
